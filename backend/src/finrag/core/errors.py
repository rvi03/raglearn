...
Every error raised by finrag derives from :class:`FinragError`, so callers
...
class FinragError(Exception):
    """Base class for all finrag errors."""


class ConfigError(FinragError):
    """Raised when configuration is missing, malformed, or fails validation."""


class RegistryError(FinragError):
    """Raised on adapter registry misuse (e.g. duplicate registration)."""


class AdapterNotImplementedError(FinragError):
