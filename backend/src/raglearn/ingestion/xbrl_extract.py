...
from raglearn.core.types import FactOrigin, FinancialFact, RawDocument
...
def _dimensions(context: object) -> str | None:
    """Render a context's explicit dimensions as a stable ``axis=member`` string."""
    dims = context.qnameDims  # type: ignore[attr-defined]
    if not dims:
        return None
    parts = [
        f"{axis}={value.memberQname}"
...
def extract_facts(instance_path: Path, filing_id: str) -> list[FinancialFact]:
    """Extract deduplicated numeric facts from a local XBRL instance.

    The instance's schema and linkbases must sit beside it on disk; Arelle
    resolves them from the document's references. Non-numeric facts (DEI text
    like the entity name) are left to metadata extraction; nil and contextless
    facts are dropped. Facts repeated across statements collapse to one.

    Args:
      instance_path: Path to the instance document (inline-XBRL ``.htm`` or a
        standalone instance ``.xml``).
      filing_id: Identifier the facts are attributed to (e.g. the accession).

    Returns:
      The filing's numeric facts.
    ...
    try:
        ...
        cntlr.close()
    logger.info("extracted %d XBRL facts from %s", len(facts), filing_id)
    return facts
...
class ArelleXbrlExtractor:
    ...
    numeric facts. The instance document is what reaches here; its schema and
    linkbases are pulled in alongside it.
    """
    ...
    def extract(self, document: RawDocument) -> Iterator[FinancialFact]:
        """Yield the numeric facts of the XBRL filing the document belongs to.
        ...
        Yields:
          The filing's deduplicated numeric facts, or nothing if the bundle is
          not yet complete (a later sibling event will retry).
        """
        filing_id = _filing_id(document.doc_id)
        with self._assembler.materialize(document) as instance_path:
            if instance_path is None:
                return  # bundle incomplete; deferred until its siblings arrive
            facts = extract_facts(instance_path, filing_id)
        yield from facts
